...
(b) The $i$-th component of $\mathrm{LN}(\mathbf{u})$ is $\gamma_i(u_i - \mu)/\sigma + \beta_i$.
...
All models use Pre-LN placement \citep{xiong2020layer}, BPE tokenization (16K vocabulary), sequence length 256, AdamW optimizer with learning rate $3 \times 10^{-4}$, cosine schedule with 1500 warmup steps, weight tying, and gradient clipping at 1.0.
...
\cmidrule(lr){3-4}
\textbf{Model} & \textbf{Params} & \textbf{WikiText-103} & \textbf{OpenWebText} & \textbf{Rel.\ $\Delta$ (WT)} \\
\midrule
Standard ($d{=}256$)              & 7.4M & $72.2 \pm 0.3$ & $114.9 \pm 0.5$ & --- \\
Twicing ($d{=}256$)               & 7.4M & $69.6 \pm 0.1$ & $110.7 \pm 0.3$ & $-3.6\%$ \\
Standard ($d{=}288$, param-fair)  & 8.8M & $69.0 \pm 0.1$ & $110.2 \pm 0.5$ & $-4.4\%$ \\
Gradient-boosted ($M{=}2$) & 8.7M & $\mathbf{67.9 \pm 0.1}$ & $\mathbf{108.5 \pm 0.9}$ & $\mathbf{-6.0\%}$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:main_results} shows the results.
On WikiText-103, gradient-boosted attention achieves a test perplexity of $67.9$, improving over standard attention by $4.3$ points ($6.0\%$ relative).
...
On OpenWebText, the same pattern holds: gradient-boosted attention achieves $108.5$ vs.\ $114.9$ for standard ($5.6\%$ relative improvement) and $110.2$ for the parameter-fair baseline ($1.5\%$ improvement).
Standard deviations are tight on both benchmarks, confirming stability across random seeds.
...
The jump from 1 to 2 rounds ($+13.5$ percentage points) accounts for the vast majority of the improvement.
Additional rounds yield diminishing returns, with accuracy plateauing around $M{=}4$.
...
\midrule
Accuracy (\%) & 41.3 & 54.8 & 55.1 & 56.4 & 55.8 \\
$\Delta$ from $M{=}1$ & --- & +13.5 & +13.8 & +15.1 & +14.5 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Accuracy (\%) & 54.3 & 55.0 & 55.2 & 48.3 \\
$\Delta$ from baseline & +6.0 & +6.7 & +6.9 & --- \\
\bottomrule
\end{tabular}
\end{table}
...
Across configurations ranging from $(d{=}32, K{=}8)$ to $(d{=}128, K{=}32)$, the improvement over the single-round baseline increases with the number of patterns and with moderate noise levels ($\sigma \leq 0.5$): at $\sigma{=}0.3$ the gain reaches $+18.7$ percentage points for $(d{=}64, K{=}16)$ and $+15.7$ for $(d{=}128, K{=}32)$, compared with $+1.9$ for $(d{=}32, K{=}8)$.
...
Hardware & 1$\times$ NVIDIA RTX 2000 Ada (16GB) (WikiText-103) \\
 ...
 & Apple M1 Pro (MPS) (synthetic ablations) \\
...
\caption{Retrieval accuracy for standard vs.\ gradient-boosted attention ($M{=}2$, MLP gate) across configurations. Numbers above bars indicate the accuracy improvement in percentage points. The benefit of boosting increases with problem dimensionality and is largest at moderate noise levels.}
